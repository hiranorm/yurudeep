<script lang="ts">
  ...
  let lineResults = $state<CharResult[][]>(lines.map(l => Array(l.replace(/[　 ]/g, '').length + l.split('').filter(c => c === '　' || c === ' ').length).fill(null) as CharResult[]));
  ...
  const totalCharsAll = $derived(
    lines.reduce((acc, l) => acc + l.split('').filter(c => c !== '　' && c !== ' ').length, 0)
  );

  // Chars in current line excluding spaces
  function getNonSpaceIndices(line: string): number[] {
    return line.split('').map((c, i) => ({ c, i })).filter(({ c }) => c !== '　' && c !== ' ').map(({ i }) => i);
  }
  ...
  async function processTyped(typed: string) {
    for (const ch of typed) {
      ...
      while (
        ...
        (currentLine[currentCharIndex] === '　' || currentLine[currentCharIndex] === ' ')
      ) {
        currentCharIndex++;
      }
