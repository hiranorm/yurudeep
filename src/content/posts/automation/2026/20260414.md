...
ユーザー補助がデスクトップで83に落ちていた原因はFuwariテーマ側のARIA実装の問題だった。具体的には3点。

- `LightDarkSwitch`：`role="menu"` の div の子ボタンに `role="menuitem"` が欠けていた
- `Search`：装飾用の検索アイコンに `aria-hidden="true"` が付いておらず、コントラスト違反として検出されていた
- `Pagination`：前/次ページが無効状態のとき `aria-label` が `null` になりリンクに識別名がなかった

Claude Codeと一緒にこの3ファイルを修正してデプロイしたところ、**デスクトップのユーザー補助が83 → 100に改善**した。
