...
![Foundation Models — Apple's in-app AI strategy just changed](/blogimg/foundation-models-ja/slide-01.webp)
...
![2025は単一エンジン、2026は抽象化レイヤと複数バッキング、同じ LanguageModelSession を別モデルが背後で駆動](/blogimg/foundation-models-ja/slide-02.webp)
...
つまり、同じ `LanguageModelSession` を別モデルが背後で駆動する、という形にAPIが揃った。ここから先は、その骨格を順に分解していく。

## 主役は `LanguageModel` プロトコル

![主役はLanguageModelプロトコル — System / PCC は準拠済み、サードパーティはLanguageModelExecutorを実装してSwiftパッケージで配布](/blogimg/foundation-models-ja/slide-03.webp)

全ての変更の幹はここ一本。`LanguageModel` というSwiftプロトコルが新設され、これに準拠するモデルなら何でも `LanguageModelSession` のバックに置ける、という設計になった^1 ^2。`SystemLanguageModel` ／ `PrivateCloudComputeLanguageModel` は標準で準拠済み。サードパーティ実装は `LanguageModelExecutor` を実装してSwiftパッケージとして配布される。

セッション側のロジックは1度書けば、その下に差し込むモデル（バッキング）を後から差し替えできる。スライドのキャッチコピーで言う conformance, not dependency（依存ではなく準拠）の意味はここにある。

## 呼び出し側は同一。`model:` を変えるだけ

![呼び出し側は同一、model: を変えるだけで切り替え。プロトタイプはオンデバイス → 重いクエリだけクラウドへ、を依存変更だけで](/blogimg/foundation-models-ja/slide-04.webp)
...
![サードパーティ frontier モデル — Claude/Gemini が公式Swift Packageで配布、OAuth+Keychainで認証](/blogimg/foundation-models-ja/slide-05.webp)
...
Swift Package Manager経由で組み込むので、ダウンストリームのコードは触らずに済む。トークン課金はキャッシュ・推論トークン込みで計測可能。

サーバモデルは、データがApple境界の外に出る唯一の場所でもある。だから default ではなく意図的な選択として置かれている、というのがAppleの説明の立て付けだ。

## マルチモーダル ＆ 組み込みツール

![マルチモーダル & 組み込みツール — 画像入力・OCRTool・BarcodeReaderTool・Spotlight検索ツールでローカルRAGが2行](/blogimg/foundation-models-ja/slide-06.webp)
...
![Core AI — 任意モデルをオンデバイスで。高レベル=Foundation Models／低レベル=Core AI、大半の開発者は前者で足りる](/blogimg/foundation-models-ja/slide-07.webp)
...
![Python SDKでSwiftの外からも同じモデルを呼べる、Linuxにも対応](/blogimg/foundation-models-ja/slide-08.webp)
...
![Dynamic Profiles — マルチエージェントの布石。1セッション内で instructions／tools／モデルを宣言的に切替、.light / .moderate / .deep をContextOptionsで指定](/blogimg/foundation-models-ja/slide-09.webp)
...
![コスト構造 — オンデバイス¥0、Small Business Program で200万MAU未満ならPCCのクラウドAPI費もゼロ。コンテキストはSystem 4096→8192／PCC 32768](/blogimg/foundation-models-ja/slide-10.webp)
...
![まとめ ＆ リンク — 幹：モデルは差し替え可能な部品／動き方：オンデバイスで作り必要な所だけ PCC・Claude・Gemini／予告：2026年夏オープンソース化](/blogimg/foundation-models-ja/slide-11.webp)
